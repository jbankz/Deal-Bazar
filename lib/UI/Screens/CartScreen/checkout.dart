...
class _CheckoutState extends State<Checkout> {
  ...
  _table(BuildContext context) {
    var price = widget.product.first.price!.replaceAll(r'$', '');
    double productPrice = double.parse(price) * 89.5;
    ...
    return DataTable(
        ...
        // ignore: prefer_const_literals_to_create_immutables
        rows: [
          for (int p = 0; p < widget.product.length; p++)
            DataRow(
              cells: [
                DataCell(
                  SizedBox(
                    child: Text(
                      widget.product[p].title!,
                ...
                DataCell(
                  Text(
                    widget.product[p].quantity.toString(),
